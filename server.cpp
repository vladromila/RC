...
void updateResponse(char res[])
{
    int l = strlen(res);
    char t[l + 4];
    snprintf(t, sizeof(t), "%d", strlen(res));
    strcat(t, res);
    strcpy(res, t);
}
...
void getLoggedUsers(int write_fd)
{
    if (username[0] != '\0')
    {
        ...
        updateResponse(toSendResponse);
        write(write_fd, toSendResponse, strlen(toSendResponse));
    }
    else
    {
        char toSendResponse[100];
        strcpy(toSendResponse,"ERROR: You need to be authenticated in order to run this command.\n");
        updateResponse(toSendResponse);
        write(write_fd, toSendResponse,strlen(toSendResponse));
    }
}
